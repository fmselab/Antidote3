...
#ifdef USE_REQ_MSG
			// AB: Customized messages to allow the communication between Antidote and ProTest
			DEBUG("--- USING Customized Messages ---\n");
			int choice = ntohs(*((uint16_t *) stream->buffer_cur));
			//req_assoc_rel
			if (choice == 0xEB00 && USE_REQ_MSG) {
		        DEBUG("\n[Customized Msg]: req_assoc_rel\n");			
		        req_association_release(ctx);
	        }
            //req_assoc_abort
	        else if (choice == 0xEA00 && USE_REQ_MSG) {
		        DEBUG("\n[Customized Msg]: req_assoc_abort\n");
		        communication_fire_evt(ctx, fsm_evt_req_assoc_abort, NULL);
	        }
            //req_agent_supplied_known_configuration
	        else if (choice == 0xE800 && USE_REQ_MSG) { 
		        DEBUG("\n[Customized Msg]: req_agent_supplied_known_configuration\n");
		        FSMEventData data;
		        data.received_apdu = &last_apdu;
		        data.choice = FSM_EVT_DATA_CONFIGURATION_RESULT;
		        data.u.configuration_result = ACCEPTED_CONFIG;	
		        communication_fire_evt(ctx, fsm_evt_req_agent_supplied_known_configuration, &data);
	        }		
	        //req_agent_supplied_unknown_configuration
	        else if (choice == 0xE900 && USE_REQ_MSG) {
                        DEBUG("\n[Customized Msg]: req_agent_supplied_unknown_configuration\n");
		        FSMEventData data;
		        data.received_apdu = &last_apdu;
		        data.choice = FSM_EVT_DATA_CONFIGURATION_RESULT;
		        data.u.configuration_result = STANDARD_CONFIG_UNKNOWN;
		        communication_fire_evt(ctx, fsm_evt_req_agent_supplied_unknown_configuration, &data);
	        }
			//terminate server
			else if (choice == 0xEE00 && USE_REQ_MSG) {
				DEBUG("\n[Customized Msg]: Terminate 11073 Server.\n");
				communication_get_state(ctx);
				communication_get_state_name(ctx);
				communication_cancel_connection_loop();
				manager_stop();
				manager_finalize();
				exit(1);
			}
...
#ifdef USE_REQ_MSG
#else
		DEBUG(" processing agent's apdu");
		communication_process_apdu_agent(ctx, apdu);
#endif
	} else {
		DEBUG(" processing manager's apdu");
